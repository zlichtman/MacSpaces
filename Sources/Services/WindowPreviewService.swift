    ...
    nonisolated private static func capture(_ window: SCWindow) async -> NSImage? {
        ...
        do {
            ...
            if #available(macOS 14.0, *) {
                ...
                // Ventura can still enumerate and activate exact windows, but
                // its public ScreenCaptureKit API has no one-shot screenshot
                // operation. Keep the browser functional with app icons and
                // titles rather than running a persistent capture stream.
                return nil
            }
