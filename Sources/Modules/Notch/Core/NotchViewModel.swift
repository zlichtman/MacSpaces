...
final class NotchViewModel: ObservableObject {
    ...
    var expandedSize: CGSize {
        ...
        if settings.widgets.isEmpty {
            return CGSize(
                ...
                height: settings.showTeleprompterBar ? 216 : 170
            )
        }
        ...
        let height = baseHeight + (settings.showTeleprompterBar ? 46 : 0)
    ...
    /// Something was dragged over the collapsed notch: open the shelf so it can be dropped.
    func dragEntered() {
        guard state == .collapsed else { return }
        expand(to: .tray)
    }
