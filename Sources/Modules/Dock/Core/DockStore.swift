...
final class DockStore: ObservableObject {
    ...
    /// Restores the shipped OpenDock configuration and its single empty
    ...
    func resetToDefaults() {
        ...
        let profile = DockProfile(id: UUID(), name: "My Dock", widgets: [])
        profiles = [profile]
        activeProfileID = profile.id
        position = .bottom
        tileSize = 72
        ...
        windowPreviewDelay = 0.18
        windowPreviewLimit = 6
        isDockVisible = true
        flushPersistence()
    }
